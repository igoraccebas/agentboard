...
**`agentboard init` does NOT make project-specific decisions.** It only copies the skeleton + drops an activation-mode `GEMINI.md` (or `CLAUDE.md`). All project-specific content (stack detection, architecture description, conventions, decisions, status) is written by the LLM during activation by reading the user's actual codebase and interviewing them briefly.
...
- The activation prompt (`templates/root/GEMINI.md.template`) is the most important file in the repo. It's the instructions the LLM follows to do the activation.
...
### When to edit `templates/root/GEMINI.md.template` (or `CLAUDE.md.template`)

- Changing the activation protocol (scan steps, interview questions, fill steps)
- Adding manifest files the LLM should look for
- Changing what the steady-state `GEMINI.md` should look like after activation
